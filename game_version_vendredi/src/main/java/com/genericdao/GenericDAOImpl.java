...
public class GenericDAOImpl<T> {
	...
	protected void handleDaoOpError(Transaction tx, RuntimeException ex) {
		...
		LOGGER.error("erreur d'annulation de la transaction é cause de l'exception : " + ex);

		// On remonte l'exception (faut pas cacher les problémes)
	...
	/**
	 * Permet de savoir si la session a déjé une transaction active (déjé
	...
	protected boolean anActiveTransactionExist(Session s) {
		if (s != null && s.getTransaction() != null)
			// retourne true si la session a déjé une transaction active
	...
	public Long create(T o) {
		...
		// Si une transaction est déjé démarré (typiquement par un filtre ou la
		// couche service métier)
		if (anActiveTransactionExist(s)) {

			LOGGER.debug("le DAO utilise la transaction  déjé démarrée dans les couches supérieures");

			// Dans ce cas on appéle directement la méthode hibernate
			id = (Long) s.save(o);
			...
			try {
				...
				// En cas de problémes on annule la transaction
				// et on trace
	...
	/**
	 * Utilisé pour mettre é jour une entité détachée (ratache un objet é la
	...
	public void update(T o) {
		...
		if (anActiveTransactionExist(s)) {
			...
			try {
				...
				// En cas de problémes en annule la transaction
				handleDaoOpError(tx, ex);
			} finally {
				// Fermer la session s'elle est encore ouverte
				closeSession(s);
	 ...
	 * nombre de résultats é retourner
	 * 
	 * @return
	 * @throws EntityNotFoundException
	 */
